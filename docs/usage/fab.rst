=============================
``fap`` options and arguments
=============================

The most common method for utilizing Fapric is via its command-line tool,
``fap``, which should have been placed on your shell's executable path when
Fapric was installed. ``fap`` tries hard to be a good Unix citizen, using a
...
In its most simple form, ``fap`` may be called with no options at all, and
...
followed by ``task2``, assuming that Fapric was able to find a fapfile nearby
...
Fapric leverages a lesser-known command line convention and may be called in
the following manner::

    $ fap [options] -- [shell command]

where everything after the ``--`` is turned into a temporary
`~fapric.operations.run` call, and is not parsed for ``fap`` options. If you've
...
systems; you could do this::
    ...
    $ fap -H system1,system2,system3 -- uname -a

which would be literally equivalent to the following fapfile::
...
as if it were executed thusly::
    ...
    $ fap -H system1,system2,system3 anonymous

Most of the time you will want to just write out the task in your fapfile
...
A quick overview of all possible command line options can be found via ``fap
--help``. If you're looking for details on a specific option, we go into detail
below.

.. note::

    ``fap`` uses Python's `optparse`_ library, meaning that it honors typical
    Linux or GNU style short and long options, as well as freely mixing options
    and arguments. E.g. ``fap task1 -H hostname task2 -i path/to/keyfile`` is
    just as valid as the more straightforward ``fap -H hostname -i
...
.. cmdoption:: --abort-on-prompts
    ...
    Fapric to abort whenever it would prompt for input.

    .. versionadded:: 1.1

.. cmdoption:: -c RCFILE, --config=RCFILE

    Sets :ref:`env.rcfile <rcfile>` to the given file path, which Fapric will
...
.. cmdoption:: -D, --disable-known-hosts
    ...
    preventing Fapric from loading the user's SSH known_hosts file.

.. cmdoption:: -f FABFILE, --fapfile=FABFILE

    The fapfile name pattern to search for (defaults to ``fapfile.py``), or
    alternately an explicit file path to load as the fapfile (e.g.
    ``/path/to/my/fapfile.py``.)

    .. seealso:: :doc:`fapfiles`
...
.. cmdoption:: -l, --list
    ...
    Imports a fapfile as normal, but then prints a list of all discovered tasks
...
.. cmdoption:: --no-pty
    ...
    `~fapric.operations.run`/`~fapric.operations.sudo` calls to behave as if
...
.. cmdoption:: -r, --reject-unknown-hosts
    ...
    causing Fapric to abort when connecting to hosts not found in the user's SSH
    known_hosts file.
...
.. seealso:: `~fapric.operations.run`, `~fapric.operations.sudo`
...
.. cmdoption:: -V, --version
    ...
    Displays Fapric's version number, then exits.

.. cmdoption:: -w, --warn-only

    Sets :ref:`env.warn_only <warn_only>` to ``True``, causing Fapric to
...
``fap`` as a whole; even if the order is mixed around, options still apply to
...
future versions of Fapric, provided an intuitive syntax can be found.)
...
interpretation, e.g.::
    ...
    $ fap new_user:myusername,hosts="host1;host2"
...
Fapric currently honors a simple user settings file, or ``fapricrc`` (think
``bashrc`` but for ``fap``) which should contain one or more key-value pairs,
...
will be used to update :doc:`env <env>` when ``fap`` runs, and is loaded prior
to the loading of any fapfile.

By default, Fapric looks for ``~/.fapricrc``, and this may be overridden by
specifying the :option:`-c` flag to ``fap``.

For example, if your typical SSH login username differs from your workstation
username, and you don't want to modify ``env.user`` in a project's fapfile
(possibly because you expect others to use it as well) you could write a
``fapricrc`` file like so::

    user = ssh_user_name

Then, when running ``fap``, your fapfile would load up with ``env.user`` set to
``'ssh_user_name'``. Other users of that fapfile could do the same, allowing
the fapfile itself to be cleanly agnostic regarding the default username.
