...
While Fapric can be used for many shell-script-like tasks, there's a slightly
unintuitive catch: each `~fapric.operations.run` or `~fapric.operations.sudo`
call has its own distinct shell session. This is required in order for Fapric
...
If that were a shell script, the second `~fapric.operations.run` call would
...
Fapric provides a convenient shortcut for this specific use case, in fact:
...
other such files, including system-wide ones.) Fapric's default mode of
operation involves executing the Bash shell in "login mode", which causes these
files to be executed.

Because Fapric also doesn't bother asking the remote end for a tty by default
...
Why can't I run programs in the background with ``&``? It makes Fapric hang.
============================================================================

Because Fapric executes a shell on the remote end for each invocation of
...
in turn prevents Fapric from continuing on with its own execution.
...
While Fapric is written with ``bash`` in mind, it's not an absolute
...
for Fapric to always accurately detect the type of authentication needed. We
...
incorrect username. In this situation, Fapric will prompt you with "Please
...
Is Fapric thread-safe?
======================

Currently, no, it's not -- the present version of Fapric relies heavily on
shared state in order to keep the codebase simple. However, there are definite
plans to update its internals so that Fapric may be either threaded or
