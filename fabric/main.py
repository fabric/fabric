...
def main():
    ...
    executes all commands in the order given.
    """
    try:
        try:
            ...
            # Load settings from user settings file (updates env)
            ...
            # Load commands from fabfile (note: this will naturally include all
            # visible callables, including ones the fabfile itself imports from
            # elsewhere)
            imp = __import__(fabfile)
            commands = dict(filter(lambda x: callable(x[1]), vars(imp).items()))
