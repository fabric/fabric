...
def _socket_server(local_address):
...
def _run_tunnel_server(finished, sock, open_channel):
    ...
    while not finished.is_set():
        ...
        channel = open_channel(local_addr)
...
class TunnelManager(ExceptionHandlingThread):
    ...
    def _run(self):
        ...
        with _socket_server(self.local_address) as sock:
            _run_tunnel_server(
...
class _TunnelSockManager(ExceptionHandlingThread):
    """
    Thread subclass for tunnelling connections over SSH between two endpoints.
